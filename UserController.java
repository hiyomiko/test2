...
import models.ebs.PaymentAccount;
import models.ebs.PaymentAccountDetail;
...
public class UserController extends Controller {
	...
	public static void processUpload() {
		...
		List<PaymentAccount> paymentAccounts = JPA.em().createQuery(
				"SELECT distinct p FROM PaymentAccount p LEFT JOIN FETCH p.paymentAccountDetails d WHERE p.merchantNumber IS NOT NULL")
				.getResultList();

		// 結果をログに出力 (確認用)
		System.out.println("件数: " + paymentAccounts.size());

		// 結果をコンソールに出力 (確認用)
		for (PaymentAccount paymentAccount : paymentAccounts) {
			System.out.println("Contract Number: " + paymentAccount.id.contractNumber);
			System.out.println("Account Type: " + paymentAccount.id.accountType);
			System.out.println("Start Date: " + paymentAccount.id.startDate);
			System.out.println("Merchant Number: " + paymentAccount.merchantNumber);

			if (paymentAccount.paymentAccountDetails != null) {
				for (PaymentAccountDetail paymentAccountDetail : paymentAccount.paymentAccountDetails) {
					System.out.println("  Contract Number: " + paymentAccountDetail.id.contractNumber);
					System.out.println("  Detail Number: " + paymentAccountDetail.id.detailNumber);
				}
			} else {
		...
		// マーチャント番号をキーとする Map を作成
		Map<String, PaymentAccount> paymentAccountMap = new HashMap<>();

		// paymentAccounts リストを反復処理し、Map に格納
		for (PaymentAccount paymentAccount : paymentAccounts) {
			paymentAccountMap.put(paymentAccount.merchantNumber, paymentAccount);
		}

		// Map の内容を確認 (ログ出力)
		for (Map.Entry<String, PaymentAccount> entry : paymentAccountMap.entrySet()) {
			String merchantNumber = entry.getKey();
			PaymentAccount paymentAccount = entry.getValue();
		...
		MessageResolver testMessageResolver = new ResourceBundleMessageResolver(
				ResourceBundle.getBundle("SuperCsvMessages", new EncodingControl("UTF-8")));
